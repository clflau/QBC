...
pdf(file="C:/Users/Tharsis/Documents/GitHub/QBC/Normal_hist.pdf", width=4,height=7); 
#open the file
...
dev.off() #shuts off current output device, essentially ending the pdf file
...
#plot histograms of both on same panel and save to a file:
pdf(file="C:/Users/Tharsis/Documents/GitHub/QBC/Normal_hist.fancy.pdf", width=4,height=7); 
#open the file
...
pdf(file="C:/Users/Tharsis/Documents/GitHub/QBC/Normal_density.pdf", width=6,height=6); #open the file
...
legend(-15,0.4,c("sigma=1","sigma=3"),lwd=4,col=c(2,4),cex=1.5) #put a legend on, cex= control relative sizes
...
#boxplot:
pdf(file="C:/Users/Tharsis/Documents/GitHub/QBC/Normal_boxplot.pdf", width=6,height=6); #open the file

par(mfrow=c(1,1), mar=c(4, 4, 3, 2)) #sets plotting area and margins

boxplot(cbind(s1,s3),names=c("Sigma=1","Sigma=3"),main="Draws from a normal distribution",col=c(2,4))
#cbind() takes vectors/matrices/dataframes and put them into columns; rbind does this for rows
...
bins<-seq(-12,12,by=1) #set bins so R does not use the default bins
...
#now make the plot:
pdf(file="C:/Users/Tharsis/Documents/GitHub/QBC/normal_barplot.pdf", width=6,height=6); #open the file

par(mfrow=c(1,1), mar=c(4, 4, 3, 2)) #sets plotting area and margins

barplot(rbind(counts_s1,counts_s3),col=c(2,4),beside=T,names.arg=seq(-12,11,by=1),xlab="Value",ylab="Count")
